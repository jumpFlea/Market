							...
							<div class="collect-list-item">
								<div class="collect-num-box">
									<p class="collect-string">
										<span class="collect-item-txt">商品分类:</span> <span class=""><a href="">${g.attribute}</a></span>
									</p>
								</div>
								<div class="collect-item-detail">
									...
									<div class="right-button-group pull-right">
										<span class="button-item"> <a href="quickBuy.action?&item.price=${g.price}&item.gid=${g.id}&item.uid=${user.uid}&item.goods_number=1" class="button current">立即购买</a></span> <span
											class="button-item"><a href="javascript:alert('已添加通知，我们商品将在降价时通知您！');" class="button">降价通知</a></span> <span class="button-item"><a href="removeFavorite.action?item.gid=${g.id}"
											class="button">取消收藏</a></span>
									</div>
									<div class="main-detail-box">
										<h3 class="collect-title">
											<a href="">${g.name}</a>
										</h3>
										<p class="size-color">
										...
										<p class="price-wrap margin-t-10">
											<span class="unit-price-wrap"> ￥<span class="unit-wrap price">${g.price}</span>
											</span>
										</p>
									</div>
								</div>
